...
short int* create3Dmodel(int Nbins, float length_voxel) {
...
//    double T[Nbins][Nbins][Nbins][3];
    ...
    //    int radius = 5;
    float gap = 0.25e-2; //1cm
    double sphere_center[3] = { Nbins - gap / length_voxel - 87,Nbins * 0.5,Nbins * 0.5 };
    ...
    for (i = 0; i < Nbins;i++) {
        for (j = 0;j < Nbins;j++) {
            for (k = 0;k < Nbins;k++) {
                ...
                if ((pow((i - sphere_center[0]), 2) / pow(87, 2) + pow((j - sphere_center[1]), 2) / pow(113, 2)
                    + pow((k - sphere_center[2]), 2) / pow(87, 2) <= 1) && (pow((i - sphere_center[0]), 2) / pow(85, 2)
                        + pow((j - sphere_center[1]), 2) / pow(111, 2)
                        + pow((k - sphere_center[2]), 2) / pow(85, 2) > 1)&&(i > sphere_center[0])) {//&&(i > sphere_center[1]): noskin-skin
                    *(T + i * Nbins * Nbins + j * Nbins + k) = 1;

                    A++;

                }
                else if ((pow((i - sphere_center[0]), 2) / pow(85, 2) + pow((j - sphere_center[1]), 2) / pow(111, 2)
                    + pow((k - sphere_center[2]), 2) / pow(85, 2) <= 1) && (pow((i - sphere_center[0]), 2) / pow(30, 2)
                        + pow((j - sphere_center[1]), 2) / pow(87, 2)
                        + pow((k - sphere_center[2]), 2) / pow(30, 2) > 1) ) {
                    *(T + i * Nbins * Nbins + j * Nbins + k) = 2;

                    B++;
                }
                else if ((pow((i - sphere_center[0]), 2) / pow(30, 2)
                    + pow((j - sphere_center[1]), 2) / pow(87, 2)
                    + pow((k - sphere_center[2]), 2) / pow(30, 2) <= 1))
                {
...
short int* create3Dmodel_lr(int Nbins,  float length_voxel, int mag) { //lr: low resolution
...
//    double T[Nbins][Nbins][Nbins][3];
    ...
    //    int radius = 5;
    float gap = 0.25e-2; //1cm
    length_voxel = length_voxel * mag;
    double sphere_center[3] = { Nbins - gap / length_voxel - 87/mag,Nbins * 0.5,Nbins * 0.5 };
    int radius = int(Nbins * 0.25);
    ...
    for (i = 0; i < Nbins;i++) {
        for (j = 0;j < Nbins;j++) {
            for (k = 0;k < Nbins;k++) {
                ...
                if ((pow((i - sphere_center[0]), 2) / pow(87/mag, 2) + pow((j - sphere_center[1]), 2) / pow(113/mag, 2)
                    + pow((k - sphere_center[2]), 2) / pow(87/mag, 2) <= 1) && (pow((i - sphere_center[0]), 2) / pow(85/mag, 2)
                        + pow((j - sphere_center[1]), 2) / pow(111/mag, 2)
                        + pow((k - sphere_center[2]), 2) / pow(85/mag, 2) > 1) && (i > sphere_center[0]/mag)) {//&& (i > 150/mag
                    *(T + i * Nbins * Nbins + j * Nbins + k) = 1;

                    A++;

                }
                else if ((pow((i - sphere_center[0]), 2) / pow(85/mag, 2) + pow((j - sphere_center[1]), 2) / pow(111/mag, 2)
                    + pow((k - sphere_center[2]), 2) / pow(85/mag, 2) <= 1) && (pow((i - sphere_center[0]), 2) / pow(30/mag, 2)
                        + pow((j - sphere_center[1]), 2) / pow(87/mag, 2)
                        + pow((k - sphere_center[2]), 2) / pow(30/mag, 2) > 1)) {
                    *(T + i * Nbins * Nbins + j * Nbins + k) = 2;

                    B++;
                }
                else if ((pow((i - sphere_center[0]), 2) / pow(30/mag, 2)
                    + pow((j - sphere_center[1]), 2) / pow(87/mag, 2)
                    + pow((k - sphere_center[2]), 2) / pow(30/mag, 2) <= 1))
                {
                    *(T + i * Nbins * Nbins + j * Nbins + k) = 3;
...
int main()
{
    ...
    short int* T_lr = create3Dmodel_lr(int(Nbins / mag), length_voxel, mag);
